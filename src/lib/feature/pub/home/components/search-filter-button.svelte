<script lang="ts">
    ...
    $effect(() => {  searchFilterStore.setFilter({priceMin: priceMinValue}); });
    $effect(() => {  searchFilterStore.setFilter({priceMax: priceMaxValue}); });
    $effect(() => {  searchFilterStore.setFilter({province: provinceValue}); });
    $effect(() => { searchFilterStore.setFilter({municipality: municipalityValue}); });
...
<Popover.Root>
    <Popover.Trigger
            class={buttonVariants({ variant: "outline", class: "flex items-center gap-2 bg-brand rounded-full hover:text-white" })}>
