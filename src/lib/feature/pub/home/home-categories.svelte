...
{:else}
    <div class="w-full py-3 md:py-5 relative">
        <Carousel.Root class="w-full relative flex items-center">
            <Carousel.Content class="flex-1 w-screen">
                <Carousel.Item class="basis-auto cursor-pointer">
                    <CategoryItem category={ALL} {onChange} isSelected={selects.length == 0}/>
                </Carousel.Item>
                {#each categories as category (category.uuid)}
                    <Carousel.Item class="basis-auto cursor-pointer">
                        <CategoryItem isSelected={isSelected(category)} {category} {onChange}/>
                    </Carousel.Item>
                {/each}
            </Carousel.Content>
        </Carousel.Root>
