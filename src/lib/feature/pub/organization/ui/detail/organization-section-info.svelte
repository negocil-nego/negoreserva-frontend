<script lang="ts">
    ...
    import {Building03Icon, Copy01Icon, InformationSquareIcon, ShareIcon} from "@hugeicons/core-free-icons";
    ...
    const socials = $derived([
        {href: data.socialsMedia?.facebook, src: PHOTO_SOCIAL_NETWORK_FACEBOOK, alt: "Facebook"},
        {href: data.socialsMedia?.tiKtok, src: PHOTO_SOCIAL_NETWORK_TIKTOK, alt: "TikTok"},
        {href: data.socialsMedia?.instagram, src: PHOTO_SOCIAL_NETWORK_INSTAGRAM, alt: "Instagram"},
        {href: data.socialsMedia?.linkedin, src: PHOTO_SOCIAL_NETWORK_LINKEDIN, alt: "LinkedIn"},
        {href: data.socialsMedia?.youtube, src: PHOTO_SOCIAL_NETWORK_YOUTUBE, alt: "YouTube"},
    ]);
...
<section class="relative w-full">
    <div class="relative flex flex-col md:flex-row items-end justify-center  md:h-48 w-full overflow-hidden rounded-xl bg-linear-to-r from-green-400 via-green-500 to-green-600 dark:from-green-700 dark:via-green-800 dark:to-green-900">
        {#if data.image}
            <img src={data.image} alt="banner" class="absolute z-10 h-full w-full object-cover"/>
        {/if}
