<script lang="ts">
  ...
  import { useCreateAccountClient } from "$lib/feature/pub/auth/register/data/hooks/use-create-account-client";

  const service = new RegisterService();
  const saveMutation = useCreateAccountClient({ service });
  ...
  const form = superForm(CLIENT_EMPTY, {
    validators: zod4Client(clientSchema),
    SPA: true,
  });
...
<form use:enhance class="w-10/12 md:w-full mx-auto space-y-4" onsubmit={onSubmit}>
  <div class="text-center space-y-1">
    <div class="text-lg font-bold">Conta de Cliente</div>
