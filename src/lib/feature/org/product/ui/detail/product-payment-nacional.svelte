<script lang="ts">
  ...
  async function handleConfirmNacional() {
    ...
    if (!isAuthenticated && selectedPriceObj != undefined) {
      ...
      toastSuccess(
        "Produto adicionado ao carrinho! Faça login para continuar.",
      );
...
<div class="space-y-4">
  <div class="space-y-3">
    {#each NACIONAL_METHODS as method (method.id)}
      ...
      >
        <div
          class="flex size-4 items-center justify-center border-2
  ...
  {#if nacionalMethod === "referencia"}
    <div class="space-y-1 bg-muted/30 p-4">
      <p class="text-sm font-medium">Pagamento por Referência</p>
      <p class="text-xs text-muted-foreground">
        Após confirmar a reserva, será gerada uma referência bancária para
        efetuar o pagamento no Multicaixa ou num balcão.
      </p>
    </div>
  ...
  <Button
    type="button"
    class="h-10 w-full bg-brand text-white hover:bg-brand/90"
