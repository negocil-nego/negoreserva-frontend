<script lang="ts">
  ...
  let selectedPriceObj = $derived(
    data.prices?.find((p) => p.uuid === selectedPrice),
  );

  let quantity = $state(1);
  let total = $derived(
    selectedPriceObj != null && selectedPriceObj.value != null
      ? selectedPriceObj.value * quantity
      : 0,
  );

  let paymentTab = $state("nacional");
  let productSlug = $derived(page.params.slug ?? "");
  let primaryImageUrl = $derived(
    data.files?.find((f) => f.isPrimary)?.url ?? data.files?.[0]?.url,
  );

  $effect(() => {
    if (data.prices?.length > 0 && !selectedPrice) {
      selectedPrice = (data.prices.find((p) => p.isPrimary) ?? data.prices[0])
        .uuid;
    }
  });
</script>

<div class="flex h-full w-full flex-col" class:mt-15={isMarginTop}>
  {#if isTitleShow}
    <p class="mb-4 text-lg font-bold text-gray-900 dark:text-gray-100">
      Pagamento
    </p>
  {/if}
  ...
  >
    ...
    {#if selectedPriceObj && quantity > 0}
      <div
        class="text-lg font-bold text-primary flex justify-between w-full md:block md:w-auto"
      >
        <span>Total</span>
        <span>
  ...
  <div class="space-y-4">
    ...
    <Tabs.Tabs bind:value={paymentTab}>
      <Tabs.TabsList class="w-full gap-2 bg-gray-50 dark:bg-gray-900">
        {#each ["nacional", "internacional"] as tab (tab)}
          <Tabs.TabsTrigger
            value={tab}
            class="flex-1 capitalize data-[state=active]:bg-white dark:data-[state=active]:bg-gray-800"
          >
