...
<div id="product-price-section" class="bg-panel p-6 space-y-6 scroll-mt-20">
  <div class="space-y-1 border-b border-slate-100 dark:border-slate-800 pb-4">
    ...
    <p class="text-sm text-slate-500 dark:text-slate-400">
      Gerencie a estrutura de preços do produto (base, hora, dia ou noite).
    </p>
  </div>
    ...
    <div class="p-4 border border-slate-150 dark:border-slate-850 space-y-4">
      ...
      <div class="flex items-center gap-2">
        <input
          type="checkbox"
          id="isPrimaryCheck"
          bind:checked={newIsPrimary}
          class="rounded text-brand focus:ring-brand cursor-pointer"
        />
        <label
          for="isPrimaryCheck"
          class="text-xs text-slate-500 dark:text-slate-400 cursor-pointer"
          >Definir este como preço principal do produto</label
        >
      </div>
    </div>
  {/if}
  ...
  <div class="space-y-3">
    <div class="text-sm font-bold text-slate-700 dark:text-slate-350">
      Preços Registados ({prices.length})
    </div>
    ...
    {#if prices.length === 0}
      ...
      >
        Nenhum preço adicionado ainda. Adicione pelo menos um preço para o
        produto.
      </div>
    {:else}
      <div
        class="overflow-hidden border border-slate-100 dark:border-slate-800"
      >
        <table class="w-full text-left border-collapse text-sm">
          ...
          <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
            {#each prices as item, index (index)}
              <tr
                class="hover:bg-slate-50/50 dark:hover:bg-slate-900/30 text-slate-750 dark:text-slate-300 transition-colors"
              >
                <td class="p-3">
                  <span
                    class="px-2 py-0.5 text-xs font-semibold border {getBadgeColor(
                      item.type,
                    )}"
                  >
                ...
                <td
                  class="p-3 text-right font-semibold text-slate-800 dark:text-slate-100"
                >
                  ...
                  >
                    ...
                    {:else}
                      <HugeiconsIcon
                        icon={StarIcon}
                        size={18}
                        class="text-slate-300 dark:text-slate-700 hover:text-amber-300"
                      />
                    {/if}
                  </button>
                </td>
                {#if !readOnly}
                  <td class="p-3 text-center">
                    <button
                      ...
                      class="text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 p-1 hover:bg-red-50 dark:hover:bg-red-950/20 transition-colors cursor-pointer"
                    >
