<script lang="ts">
  ...
  const socials = $derived([
    { href: socialMedia?.facebook, src: PHOTO_SOCIAL_NETWORK_FACEBOOK, alt: "Facebook" },
    { href: socialMedia?.tiKtok, src: PHOTO_SOCIAL_NETWORK_TIKTOK, alt: "TikTok" },
    { href: socialMedia?.instagram, src: PHOTO_SOCIAL_NETWORK_INSTAGRAM, alt: "Instagram" },
    { href: socialMedia?.linkedin, src: PHOTO_SOCIAL_NETWORK_LINKEDIN, alt: "LinkedIn" },
    { href: socialMedia?.youtube, src: PHOTO_SOCIAL_NETWORK_YOUTUBE, alt: "YouTube" },
  ]);
...
{#snippet socialLink(href: string | null | undefined, src: string, alt: string)}
  {#if href}
    <a {href} target="_blank" rel="noopener noreferrer" class="bg-white rounded-full flex flex-col justify-center items-center p-1 h-6 w-6 md:w-8 md:h-8">
...
<section class="relative w-full">
  <div class="relative flex flex-col md:flex-row items-end justify-center md:h-48 w-full overflow-hidden rounded-xl bg-linear-to-r from-green-400 via-green-500 to-green-600 dark:from-green-700 dark:via-green-800 dark:to-green-900">
    {#if org.image}
      <img src={org.image} alt="banner" class="absolute z-10 h-full w-full object-cover" />
    {/if}

    <aside class="z-20 md:absolute top-1 right-2 p-4 flex gap-2 md:gap-3">
      <Button variant="outline" class="w-10 h-10 rounded-full bg-white dark:bg-slate-900 cursor-pointer">
    ...
    <section class="z-20 left-5 bottom-5 flex justify-center md:justify-between flex-col md:flex-row w-full items-end p-2">
      <aside class="flex flex-col md:flex-row justify-center md:justify-start items-center md:items-start gap-1 w-full lg:h-37.5 lg:gap-5">
        {#if org.logo}
          <div class="bg-white rounded-2xl w-25 lg:w-37.5 h-full">
            <img src={org.logo} alt={org.name} class="size-24 md:size-28 lg:h-full m-auto rounded-2xl object-cover" />
          </div>
        {:else}
        ...
        <div class="bg-black/5 p-1 rounded-md flex flex-col items-center md:items-start md:h-full">
          ...
          {#if org.rating}
            <RatingGenerator rating={org.rating} className="text-[20px] md:text-xl" />
          {/if}
      ...
      <aside class="flex gap-3 w-full justify-center md:justify-end md:w-auto mt-2 md:mt-0">
  ...
  <div class="absolute z-10 inset-0 bg-black/50 to-transparent rounded-md h-full"></div>
</section>
