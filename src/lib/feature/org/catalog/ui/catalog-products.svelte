<script lang="ts">
  ...
  async function loadProductsIn() {
    isLoadingIn = true;
    try {
      const result = await service.paginateCatalogProducts(
        uuidOrSlug,
        filterList,
      );
  ...
  async function loadProductsNotIn() {
    isLoadingNotIn = true;
    try {
      const result = await service.paginateCatalogProductsNotIn(
        uuidOrSlug,
        filterAdd,
      );
  ...
  async function handleSaveAdd() {
    ...
    isSavingAdd = true;
    try {
      await service.addProductsToCatalog(
        uuidOrSlug,
        addSelected.map((i) => ({
  ...
  async function handleSaveRemove() {
    ...
    isSavingRemove = true;
    try {
      await service.removeProductsFromCatalog(
        uuidOrSlug,
        removeSelected.map((i) => i.productUuid),
      );
        ...
        {:else if productsIn.length === 0}
          ...
          >
            ...
            <p class="text-muted-foreground text-sm mt-1">
              Adicione produtos através do separador "Adicionar".
            </p>
          </div>
        {:else}
          <div
          ...
          >
            {#each productsIn as product (product.uuid)}
              <button
                ...
                class="border bg-card text-card-foreground shadow-sm overflow-hidden hover:shadow-md transition-shadow text-left cursor-pointer"
              ...
              >
                <div class="aspect-video bg-muted relative overflow-hidden">
                  ...
                  {:else}
                    <div
                      class="flex items-center justify-center size-full text-muted-foreground"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="40"
                  ...
                  {#if isSelectedRemove(product.uuid)}
                    <div
                      class="absolute top-2 right-2 size-6 bg-red-500 flex items-center justify-center"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="14"
                        ...
                        fill="none"
                        stroke="white"
                        stroke-width="3"><path d="M5 12h14" /></svg
                      >
                    </div>
                  {/if}
                </div>
      ...
      <div class="space-y-4">
        <div class="flex items-center justify-between">
          ...
          <Button
            ...
            class="bg-brand text-white hover:bg-brand/90 md:absolute md:right-0 md:top-0"
          >
        ...
        {:else}
          ...
          >
            {#each productsNotIn as product (product.uuid)}
              <div
                class="border bg-card text-card-foreground shadow-sm overflow-hidden hover:shadow-md transition-shadow"
              ...
              >
                ...
                >
                  <div class="aspect-video bg-muted relative overflow-hidden">
                    ...
                    {#if isSelectedAdd(product.uuid)}
                      <div
                        class="absolute top-2 right-2 size-6 bg-brand flex items-center justify-center"
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width="14"
                  ...
                  <div class="p-4">
                    <h3 class="font-semibold truncate">{product.name}</h3>
                    <p
                      class="text-sm text-muted-foreground line-clamp-2 mt-0.5"
                    >
                      {product.description}
                    </p>
                  </div>
                ...
                {#if isSelectedAdd(product.uuid)}
                  <div class="px-4 pb-4">
                    <label
                      for={`catalog-order-${product.uuid}`}
                    ...
                    <input
                      ...
                      type="number"
                      min="1"
                      value={addSelected.find(
                        (i) => i.productUuid === product.uuid,
                      )?.order ?? 1}
                      oninput={(e) => {
                        const val = parseInt(e.currentTarget.value, 10);
                        if (!isNaN(val) && val >= 1) {
                          catalogProductStore.add.updateOrder(
                            product.uuid,
                            val,
                          );
                        }
                      }}
                      class="flex h-9 w-20 border border-input bg-transparent px-3 py-1 text-sm shadow-sm transition-colors"
                    />
                  </div>
                {/if}
              </div>
