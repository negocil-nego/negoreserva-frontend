...
<Dialog.Root bind:open>
  <Dialog.Content class="sm:max-w-106.25">
    <Dialog.Header>
      ...
      <Dialog.Description
        >Crie um novo catálogo para organizar seus produtos.</Dialog.Description
      >
    </Dialog.Header>
    <form onsubmit={handleSubmit} class="grid gap-4">
      ...
      <div class="grid gap-2">
        <label for="catalog_description" class="text-sm font-medium"
          >Descrição</label
        >
        <textarea
          ...
          required
          class="flex min-h-[80px] w-full border border-input bg-transparent px-3 py-2 text-sm shadow-sm placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
        ></textarea>
      </div>
      <div class="grid gap-2">
        ...
        {#if imagePreview}
          <div class="relative overflow-hidden border">
            ...
            <button
              type="button"
              onclick={removeImage}
              class="absolute top-1 right-1 bg-black/50 text-white size-6 flex items-center justify-center text-xs hover:bg-black/70"
            >
              ✕
            </button>
          </div>
        {:else}
          <label
            class="flex flex-col items-center justify-center h-32 border border-dashed cursor-pointer hover:bg-muted/50 transition-colors"
          >
            <svg
